...
public final class RegressionTree {
  ...
  private static void separate(
      ...
      ArrayList<Record> hi) {
    for (var record : records) {
      ((record.inputs.get(variable).number() < threshold) ? lo : hi).add(record);
    }
  }
  ...
  private static Term split(List<Record> records, Variable variable, double threshold) {
    ...
    var hi = new ArrayList<Record>();
    separate(records, variable, threshold, lo, hi);
    Term ifTrue = lo.isEmpty() ? new Number(mean(records)) : of(lo);
    Term ifFalse = lo.isEmpty() ? new Number(mean(records)) : of(hi);
  ...
  private static double threshold(List<Record> records, Variable variable) {
    ...
    for (int i = 0; i < records.size() - 1; i++) {
      var threshold =
          (records.get(i).inputs.get(variable).number()
                  + records.get(i + 1).inputs.get(variable).number())
              / 2;
