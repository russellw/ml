...
public final class TptpParser {
            ...
            switch (role) {
              ...
              default:
                throw new ParseException(
                    file, reader.getLineNumber(), role + ": invalid role for cnf");
            }
            expect(',');
        ...
        case "include":
          {
            ...
            if (tptp == null) {
              throw new ParseException(
                  file, reader.getLineNumber(), "TPTP environment variable not set");
            }
      ...
      case WORD:
        {
          ...
          if (tok == '(') {
            ...
            if (a == null) {
              var returnType = (defaultType == null) ? new TypeVariable() : defaultType;
          ...
          var a = functions.get(s);
          if (a == null) {
            var type = (defaultType == null) ? new TypeVariable() : defaultType;
  ...
  private void expect(int k) throws IOException {
    ...
    throw new ParseException(
        file, reader.getLineNumber(), string(tok, tokString) + ": '" + (char) k + "' expected");
  }
  ...
  public static Problem read(Path path, InputStream stream) throws IOException {
    ...
    // Negate conjecture
    if (problem.conjecture != null) {
      var negatedConjecture =
          new FormulaTermFrom(problem.conjecture.term().not(), problem.conjecture) {
            @Override
  ...
  private Term unaryFormulaParams() throws IOException {
    ...
    do {
      ...
      var name = tokString;
      lex();
      var type = eat(':') ? typeAtom() : Type.INDIVIDUAL;
