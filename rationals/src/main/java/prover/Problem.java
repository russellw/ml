...
public final class Problem {
  ...
  public void print(String file) throws IOException {
    ...
    writer.println("<td class=\"bordered\">" + ((cnf == null) ? "Input" : "CNF conversion"));
    writer.println(
        "<td class=\"bordered\"; style=\"text-align: right\">"
            + numberFormat.format(clauses.size()));
    writer.println("<tr>");
    ...
    writer.println(
        "<td class=\"bordered\"; style=\"text-align: right\">"
            + numberFormat.format(superposition.tautologies));
    writer.println("<tr>");
    writer.println("<td class=\"bordered\">");
    ...
    writer.println("</table>");
    var clausesGenerated =
        superposition.generatedResolution
            + superposition.generatedFactoring
            + superposition.generatedSuperposition;
    var clausesDiscarded =
        superposition.tautologies + superposition.subsumedForward + superposition.subsumedBackward;
    ...
    writer.println("<table>");
    writer.printf(
        "<caption>%d operators used for feature vector indexing: ",
        superposition.subsumption.histogram.map.size());
    writer.printf(
        "%s<sub>%s</sub>(%s<sup>-</sup>), |%s<sup>-</sup>|<sub>%s</sub>, ",
        mathLetter('d'), mathLetter('f'), mathLetter('C'), mathLetter('C'), mathLetter('f'));
    writer.printf(
        "%s<sub>%s</sub>(%s<sup>+</sup>), |%s<sup>+</sup>|<sub>%s</sub>",
        mathLetter('d'), mathLetter('f'), mathLetter('C'), mathLetter('C'), mathLetter('f'));
    writer.println();
  ...
  private void printSample(Clause clause) {
    ...
    writer.printf("<span style=\"color: #%s\">", fade ? "88e" : "00c");
    print(term, null);
    writer.print("</span>).");
    if (fade) {
      writer.print("</span>");
  ...
  public static void printSummary(String file) throws IOException {
    ...
    for (var problem : summaries) {
      writer.println("<tr>");
      writer.printf(
          "<td class=\"bordered\"><a href=\"%s.html\">%s</a>\n", problem.name, problem.name);
      writer.println(
          "<td class=\"bordered\"; style=\"text-align: right\">"
              + numberFormat.format(problem.predicates));
      writer.println(
          "<td class=\"bordered\"; style=\"text-align: right\">"
              + numberFormat.format(problem.functions));
      writer.println(
          "<td class=\"bordered\"; style=\"text-align: right\">"
              + numberFormat.format(problem.formulas));
      writer.println(
          "<td class=\"bordered\"; style=\"text-align: right\">"
              + numberFormat.format(problem.clauses));
      writer.println(
          "<td class=\"bordered\"; style=\"text-align: right\">"
              + numberFormat.format(problem.generated));
      writer.println(
          "<td class=\"bordered\"; style=\"text-align: right\">"
              + numberFormat.format(problem.discarded));
      writer.println(
          "<td class=\"bordered\"; style=\"text-align: right\">"
              + numberFormat.format(problem.final1));
