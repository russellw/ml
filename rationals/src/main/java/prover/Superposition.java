...
public final class Superposition extends Thread {
    ...
    public final PriorityQueue<Clause> unprocessed = new PriorityQueue<>(Comparator.comparingInt(Superposition::volume));
    ...
    private static Clause original(Clause clause) {
        return (clause instanceof ClauseRenamed)
               ? ((Clause) clause.from()[0])
               : clause;
    }
    ...
    private void superposition(Clause c, Clause d, int ci, Term c0, Term c1, int di, Term d0, Term d1, List<Integer> position,
                               Term a) {
    ...
    private void superposition1(Clause c, Clause d, int ci, Term c0, Term c1, int di, Term d0, Term d1, List<Integer> position,
                                Term a) {
        var map = new HashMap<Variable, Term>();
        if (!c0.unify(a, map)) {
            return;
        ...
        ((di < d.negativeSize)
         ? negative
         : positive).add(e.replaceVars(map).term());
