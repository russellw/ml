...
public final class CNF {
    ...
    private void convert(Formula formula) {
        ...
        if (!a.isomorphic(b, new HashMap<>())) {
            formula = new FormulaTermFrom(b, formula) {
                @Override
        ...
        for (var i = 1; i <= andSize(a); i++) {
            ...
            positive.clear();
            split(andGet(a, i));
            var c = new ClauseFrom(negative, positive, from) {
                @Override
                public String inference() {
                    return "split";
                }
    ...
    private Term nnf(Map<Variable, Variable> all, Map<Variable, Term> exists, boolean polarity, Term a) {
        switch (a.tag()) {
        case CONST_FALSE:
        ...
        case ALL: {
            return polarity
                   ? nnfAll(all, exists, true, a)
                   : nnfExists(all, exists, false, a);
        }
        case AND: {
        ...
        case EXISTS: {
            return polarity
                   ? nnfExists(all, exists, true, a)
                   : nnfAll(all, exists, false, a);
        }
        case NOT:
    ...
    private Term nnfAll(Map<Variable, Variable> all, Map<Variable, Term> exists, boolean polarity, Term a) {
    ...
    private Term nnfExists(Map<Variable, Variable> all, Map<Variable, Term> exists, boolean polarity, Term a) {
