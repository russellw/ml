...
public final class Subsumption {
    ...
    public boolean add(Clause c) {
        ...
        // Forward subsumption
        if (trie.findLessEqual(features(c),
            d -> {
                if (subsumed(d)) {
                    return false;
                }
        ...
        trie.forGreaterEqual(features(c),
            d -> {
                if (subsumed(d)) {
                    return;
                }
    ...
    private static DeterministicMatches deterministicMatches(Term[] c, Term[] d, Map<Variable, Term> map) {
        ...
        for (var i = 0; i < c.negativeSize; i++) {
            ...
            a.walk(
                (b, depth) -> {
                    ...
                    r[2 + NEGATIVE_OP_MAX_DEPTH * ops.length + j] = Math.max(r[2 + NEGATIVE_OP_MAX_DEPTH * ops.length + j],
                                                                             depth);
        ...
        for (var i = c.negativeSize; i < c.literals.length; i++) {
            ...
            a.walk(
                (b, depth) -> {
                    ...
                    r[2 + POSITIVE_OP_MAX_DEPTH * ops.length + j] = Math.max(r[2 + POSITIVE_OP_MAX_DEPTH * ops.length + j],
                                                                             depth);
    ...
    private Map<Variable, Term> search(Term[] c, Term[] c2, Term[] d, Term[] d2, Map<Variable, Term> map)
